...
{#if featureds.data.length > 0}
    <Section {title}>
        <ul class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5 sm:gap-10 xl:gap-5">
            {#each featureds.data as item}
                <li>
                    <Link
                        aria-label={`Ler destaque ${item.title}`}
                    ...
                    >
                        <article>
                            ...
                            <div class="relative z-10 p-3 mt-auto sm:mt-0 sm:p-0">
                                <h2 class="w-full sm:w-[50%] font-noto-sans font-extrabold text-white sm:text-suspense-aurora text-md sm:text-lg uppercase italic line-clamp-4 sm:line-clamp-5">
                                    {item.title}
                                </h2>
                            </div>
                            <img
